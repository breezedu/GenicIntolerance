...
fitinv <- stan(model_code=hiernormalinvg, data=M1_table,iter=40000,warmup=36000,chains=4)
...
plotdesvar<-function(length){
	pdf(file = "0327_40k_inverse_gamma_prior_var_density_plot.pdf")
	for (i in 1:length){
	...
	plot(density(answer$sigma),  main="density plot of sigma^2")
