...
public class D0303_checkingHomHet2{
	...
	public static void main(String[] args) throws FileNotFoundException{
		...
		//split this section into two sections, one to create all geneData objects based on the first txt document
		//``` two, update the geneData objects (sample list and control list) according to all other txt documents
		Scanner read1st = new Scanner( new File(routine + doc[0] + ".txt"));
		geneList = checkFirstDoc(read1st, doc[0], geneList);
		read1st.close();
		...
		for(int i=0; i<doc.length; i++){
			...
			geneList = checkOtherDoc(read_in, doc[i], geneList);
	 ...
	 * @return
	 */
	private static ArrayList<geneData> checkOtherDoc(Scanner read_in, String string, ArrayList<geneData> geneList) {
		// TODO Auto-generated method stub
		// ignore the first line
		read_in.nextLine();
		int line = 0;
		//check each line of data, update the gene-individual matrix
		while(read_in.hasNextLine()){
			...
			//for each individula's gene data, update geneData objects in the geneList ArrayList;
			for(int i=1; i<lineSplit.length; i++){
				
				//get the n1 or n2 
				int num = Integer.parseInt(lineSplit[i]);
				
				int temp = geneList.get(i-1).getControlList().get(line) + num;
				
				if(sample == "Control"){
					
					geneList.get(i-1).getControlList().set(line, temp);
				
				} else if (sample == "Sample") {
					
					geneList.get(i-1).getSampleList().set(line,  temp);
	 ...
	 * @return 
	 */
	private static ArrayList<geneData> checkFirstDoc(Scanner read_in, String doc, ArrayList<geneData> geneList) {
		// TODO Auto-generated method stub
		...
		System.out.println("\n ***************************\n");
		System.out.print("There are " + geneList.size() + " genes.");
